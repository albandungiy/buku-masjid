<button type="button" class="btn btn-teal bm-btn justify-content-between" data-bs-toggle="offcanvas" data-bs-target="#books" aria-controls="books">
    <!-- <div class="word-break: break-word; white-space: normal;">
        {{ $selectedBook->name }}
    </div>
    ...
    Pilih Laporan</span>&nbsp;<i class="ti">&#xea61;</i>
</button>
        ...
        <div class="d-flex flex-wrap gap-2">
            @forelse ($books as $bookItem)
                <div class="flex-shrink-0 ">
                    <a href="{{ route('public_reports.index', ['active_book_id' => $bookItem->id, 'nonce' => $bookItem->nonce]) }}" id="show-book-{{ $bookItem->id }}" class="btn {{ $bookItem->id == $selectedBook->id ? 'bm-bg-primary text-light' : 'bm-txt-primary' }}" style="border-radius: 9px; white-space: normal; word-break: break-word;">
